...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	osStatus_t status_X_pos, status_Y_pos;
	...
	if (b>0) {
	  ...
	  request_msg.channelNum = LCD_TS_YU_CHANNEL;           //Assign the analog touchscreen data(X position) to the request queue
		request_msg.Msg = result_msg_id;                      // Map the result queue ID to the request queue pointer
		osMessageQueuePut(request_msg_id,&request_msg,0u,0u); // Puts the request message into the request queue
		
		status_X_pos = osMessageQueueGet(result_msg_id,&result_msg,0u,osWaitForever); // Waits until get the digital result from result queue
		
		if(status_X_pos == osOK)
		{
			if(result_msg.channelNum == LCD_TS_YU_CHANNEL)
			{
				yu= result_msg.result;
			}
		}
		x = yu;
    ...
    request_msg.channelNum=LCD_TS_XL_CHANNEL;             //Assign the analog touchscreen data(Y position) to the request queue
		request_msg.Msg = result_msg_id;                      // Map the result queue ID to the request queue pointer
		osMessageQueuePut(request_msg_id,&request_msg,0u,0u); // Puts the request message into the request queue
		status_Y_pos = osMessageQueueGet(result_msg_id,&result_msg,0u,osWaitForever); // Waits until get the digital result from result queue
		
		if(status_Y_pos == osOK)
		{
			if(result_msg.channelNum == LCD_TS_XL_CHANNEL)
			{
				xl= result_msg.result;
			}
		}
		y = xl;
